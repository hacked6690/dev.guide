				...
				<div class="well well-sm">
					...
					<div class="row">
						<div class="col-md-12 col-lg-8">
							...
							<form action="{{ route('guideprice.update', encrypt($guideprice->id)) }}" class="smart-form" method="post">
							{{csrf_field()}}
								<fieldset>
											...
											<label class="select">
												<select class="input-sm border-0 border-bottom-1" name="language_id">
													...
													@foreach($languages as $language)
														@if($guideprice->language_id ==$language->term_id)
											...
											<label class="select">
												<select class="input-sm border-0 border-bottom-1" name="province_id">
													...
													@foreach($provinces as $province)
														@if($guideprice->province_id ==$province->term_id)
											...
											<label class="select">
												<select class="input-sm border-0 border-bottom-1" name="boolean_id">
													...
													@foreach($booleans as $boolean)
														@if($guideprice->default ==$boolean->title)
									...
									<div class="row">
										<section class="col col-6 flexibled-error">
											...
											<label class="input">
												<input type="text" name="price" value="{{$guideprice->price }}" class="input-sm border-0 border-bottom-1">
											</label>

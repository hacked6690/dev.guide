		...
		if($(".req_action"))
		{
			$(".req_action").unbind("click").on("click", function(e)
			{
				...
				$.confirm({
		    		...
		    		content: 'You are about to <span class="txt-color-'+ (action =='approved' ? 'green' :'red' ) +' font-16 font-bold">'+ action +'</span> this request, ok ?',
				    buttons: {
				        ok: {
				            text: "ok!",
				            btnClass: 'btn-primary',
								...
								$.ajax({
									...
									success: function(data)
									{
										if(!data.result)
										{
											...
											setTimeout(function(e) { window.location.reload(); }, 1300);
										}
									},
									error: function (responseData, textStatus, errorThrown) {}
								});
